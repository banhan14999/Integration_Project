        ...
        if($_POST['vacation_type'] === "Shareholder_Status"){  
            ...
            while($rows = sqlsrv_fetch_array($sql_select)){
                if($rows['Shareholder_Status'] == 1){
                    $to_date_1 += $rows['TOTAL_NUMBER_VACATION_WORKING_DAYS_PER_MONTH'];
                }
                else{
                    $to_date_2 += $rows['TOTAL_NUMBER_VACATION_WORKING_DAYS_PER_MONTH'];
                }
            }
            $output .= '
                            <div class="col-sm-6 col-xl-4">
                                <div class="bg-light rounded d-flex align-items-center justify-content-between p-4">
                                    ...
                                    <div class="ms-3">
                                        ...
                                        <h6 class="mb-0">'.$to_date_1.' Days</h6>
                                    </div>
                                </div>
                            </div>
                        ';
                ...
                <div class="col-sm-6 col-xl-4">
                    <div class="bg-light rounded d-flex align-items-center justify-content-between p-4">
                        ...
                        <div class="ms-3">
                            ...
                            <h6 class="mb-0">'.$to_date_2.' Days</h6>
                        </div>
                    </div>
                </div>
            ';
            ...
            $output = '';
            //query
            $stmt = "SELECT * from ((Employment inner join Personal on Employment.Employee_ID = Personal.Employee_ID) inner join EMPLOYMENT_WORKING_TIME on EMPLOYMENT_WORKING_TIME.EMPLOYMENT_ID = Employment.Employee_ID)";
            $sql_select = sqlsrv_query($connsqlsv,$stmt);
            //end query          
            $employees = array();
            while($rows =  sqlsrv_fetch_array($sql_select)){
